...
CREATE TABLE IF NOT EXISTS `card` (
  ...
  PRIMARY KEY (`id`),
  KEY `card_ibfk_1` (`list_id`),
  KEY `card_ibfk_2` (`color_id`),
  CONSTRAINT `card_ibfk_1` FOREIGN KEY (`list_id`) REFERENCES `list` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Listage des données de la table planner_room.invitation : ~0 rows (environ)
/*!40000 ALTER TABLE `invitation` DISABLE KEYS */;
/*!40000 ALTER TABLE `invitation` ENABLE KEYS */;

-- Listage de la structure de la table planner_room. list
CREATE TABLE IF NOT EXISTS `list` (
  ...
  CONSTRAINT `list_ibfk_1` FOREIGN KEY (`tableApp_id`) REFERENCES `tableapp` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf16 COLLATE=utf16_bin;

-- Listage des données de la table planner_room.list : ~0 rows (environ)
...
-- Listage des données de la table planner_room.participation : ~4 rows (environ)
...
INSERT INTO `tableapp` (`id`, `title`, `description`, `userApp_id`) VALUES
	...
	(3, 'Vacances', 'Organisation d&#39;un week-end au ski', 4);
...
INSERT INTO `userapp` (`id`, `nickname`, `email`, `password`, `code_change`, `role`) VALUES
	...
	(4, 'K.SCHMIDT', 'k.schmidt@gmail.com', '$argon2i$v=19$m=65536,t=4,p=1$MWIwMjVqV2Zwd284Rk53Mw$v6J4BLfUb7t/KmCv2GTZy+M2K3D+j+5IfD6DNYTi0KY', NULL, 'ROLE_USER'),
